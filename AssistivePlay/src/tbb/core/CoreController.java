...
public class CoreController {
	...
	public static CoreController sharedInstance() {
		if (mSharedInstance == null)
			mSharedInstance = new CoreController();
	...
	private void initializeReceivers() {
		...
		AssistivePlay aPlay = new AssistivePlay();
		registerIOEventReceiver(aPlay);
	...
	private void configureScreen() {
		WindowManager wm = (WindowManager) mTBBService
	...
	public boolean registerKeystrokeEventReceiver(KeystrokeLogger receiver) {
		return mKeystrokeEventReceiver != null ? mKeystrokeEventReceiver
				.add(receiver) : false;
	}
	...
	public void updateKeystrokeEventReceivers(String keystroke) {
		...
		if (mKeystrokeEventReceiver == null)
			return;
	...
	public boolean registerIOEventReceiver(IOEventReceiver ioReceiver) {
		return mIOEventReceivers != null ? mIOEventReceivers.add(ioReceiver)
				: false;
	}
	...
	public boolean unregisterIOReceiver(IOEventReceiver ioReceiver) {
		return mIOEventReceivers != null ? mIOEventReceivers.remove(ioReceiver)
				: false;
	}
	 ...
	 */
	public void updateIOReceivers(int device, int type, int code, int value,
			int timestamp) {

		if (mIOEventReceivers == null)
			return;
		for (IOEventReceiver receiver : mIOEventReceivers) {
			receiver.onUpdateIOEvent(device, type, code, value, timestamp);
		}
	 ...
	 * 
	 */
	public boolean registerAccessibilityEventReceiver(
			AccessibilityEventReceiver eventReceiver) {

		return mAccessibilityEventReceivers != null ? mAccessibilityEventReceivers
				.add(eventReceiver) : false;
	}

	public boolean unregisterEvent(AccessibilityEventReceiver eventReceiver) {

		return mAccessibilityEventReceivers != null ? mAccessibilityEventReceivers
				.remove(eventReceiver) : false;
	}

	public void updateAccessibilityEventReceivers(AccessibilityEvent event) {

		if (mAccessibilityEventReceivers == null)
			return;
		for (AccessibilityEventReceiver receiver : mAccessibilityEventReceivers) {
			if (checkEvent(receiver.getType(), event))
				receiver.onUpdateAccessibilityEvent(event);
		}
	}
	...
	public int xToScreenCoord(double x) {
		// TODO should be the commented line
		// return (int) (M_WIDTH / mTBBService.getScreenSize()[0] * x);
		return (int)x; // (M_WIDTH / 460 * x);
	}
	...
	public int currentFileId() {
		return PreferenceManager.getDefaultSharedPreferences(mTBBService)
				.getInt("preFileSeq", 0);
	}
